...
# x_0: initial sample
def slice_sampling(num_samples: int, x_0: object) -> object:
...
def render_plot(samples):
    ...
    ax.set_title("Stepping-out method. 50000 samples. x_0 = 0.5 and density proportional to\n"
                 "f(x) = (1 + sin(3x)^2) (1 + cos(5x)^4) exp(-x^2/2)")

    # Tweak spacing to prevent clipping of y label
    fig.tight_layout()
    plt.savefig('plot_examples/Normal_mixture_stepping_out', dpi=None, facecolor='w',
...
def main():
    ...
    render_plot(samples)
    print("graph saved in plot_examples/Normal_mixture_stepping_out")
